...
num_points_input = 16;
jestim_vector = linspace(-0.4,0.1,num_points_input);
...
for k = 1:num_points_gain
    ...
    VT=-55;
    parfor j=1:length(jestim_vector)
        ...
        Tburn=T/2;
        reSim=hist(s(2,s(1,:)>Tburn & s(2,:)<=1),1)/(T-Tburn);
...
save('./f_i_curve_singlecell_DT.mat', 'c','Cm','DeltaT','delta_vector',...
