...
%     plot(jestim_vector,1000*mean_i_rate,'linewidth',3,'color','red')
    plot(jestim_vector,1000*(0.8*mean_e_rate(:,thr_indx)+0.2*mean_i_rate(:,thr_indx)),'linewidth',3)
end
xlabel('Input')
ylabel('Frequency')
...
%     plot(jestim_vector,mRii,'color','red','linewidth',3)
    plot(jestim_vector,(mRee(:,thr_indx)+2*mRei(:,thr_indx)+mRii(:,thr_indx))/4,'linewidth',3)
end
xlabel('Input')
ylabel('Correlations')

%%
save('./f_i_curve_network_VT.mat', 'c','Cm','DeltaT','delta_vector',...
    'dt','EL','gL','jestim_vector','Jm','Jxm','mean_e_rate',...
    'mean_i_rate','mRee','mRei',...
    'mRii','N','Ne','Ni','num_points_input','num_points_gain',...
    'Nx','P','rx_vector','T','T1','T2','threshold_vector',...
